...
(function () {
    ...
    /* Inline-Formatierung auf bereits escapedem Text */
    function mdInline(s) {
        return s
            .replace(/`([^`\n]+)`/g, '<code>$1</code>')
            .replace(/\[([^\]\n]+)\]\((https?:[^)\s]+)\)/g,
    ...
    function parseList(lines, start, out) {
        ...
        var liOpen = false;
        function closeLi() { if (liOpen) { out.push('</li>'); liOpen = false; } }
        function openList(tag, indent) { out.push('<' + tag + '>'); stack.push({ tag: tag, indent: indent }); liOpen = false; }
        function closeList() { closeLi(); out.push('</' + stack.pop().tag + '>'); }
    ...
    function mdBlock(seg, out) {
        ...
        while (i < lines.length) {
            ...
            if (line.indexOf('|') !== -1 && i + 1 < lines.length
                ...
                out.push('<table><thead><tr>' + header.map(function (c, j) { return cell(c, j, 'th'); }).join('') + '</tr></thead>');
                out.push('<tbody>' + body.map(function (r) {
                    return '<tr>' + r.map(function (c, j) { return cell(c, j, 'td'); }).join('') + '</tr>';
                }).join('') + '</tbody></table>');
                continue;
            }
            para.push(line);
